...
- `Client/tests/control_messages_test.cpp` — same shape, pinning the scrcpy wire
  layouts in `src/stream/control_messages.h` against the bundled server 4.1:
  `UHID_CREATE` (including the vendor/product block that 2.7 did not have),
  `UHID_INPUT` — the message every keystroke rides on — `UHID_DESTROY`,
  `OPEN_HARD_KEYBOARD_SETTINGS`, `START_APP`, `SET_DISPLAY_POWER`'s boolean payload,
  `INJECT_SCROLL` with its ÷16 scale and its clamped coordinates, the
  session/config/key-frame flag bits and session-packet size parsing. This is the one part of
  the protocol whose mistakes are *silent* — a wrong `UHID_CREATE` offset kills the
  server's entire control thread while video keeps running — which is why the byte
  building lives in a header with no socket or FFmpeg dependency and the test links
  nothing at all. `ScrcpyClient` calls the same functions, so the pinned bytes are the
  shipped bytes.
...
- **Virtual display is an experiment, and it is on by default** (`Settings::m_virtual_display`, branch `virtual-display-dev-`). The PC gets its own Android display instead of the phone's picture. Three things are load-bearing and each was learned the hard way — the full record with measurements is in `VIRTUAL-DISPLAY.md`:
  - **The phone's own launcher for secondary displays is useless.** On a Pixel 9 / Android 17 its app drawer opens but tapping an app starts nothing, without even a refusal in the log. The mode therefore ships Fossify Launcher, starts it with `START_APP` and `vd_system_decorations=false`, and installs it during first-time setup over USB. Installing goes through `push_file` + `install_pushed_app`, deliberately **not** `install_app` with its `-g`. Two consequences: (1) with no launcher available `start_stream` falls back to plain mirroring and says so, because the alternative is handing the user a display that renders but starts nothing — a package named by hand in `virtual_display_app` goes through the same `is_app_installed` check before it is handed to the server, since `START_APP` has no answer the client could read (its `bool` only says the control message left the socket) and a typo would otherwise produce exactly that black display; (2) the launcher package is resolved per session into `ScrcpyClient::Config` and deliberately **never** written back to `settings.txt` — `main.cpp` holds a long-lived `Settings` copy and rewrites the whole file on every menu click, so a value stored there would live only until the next toggle. `ScrcpyClient::new_display_app()` is the authority.
  - **Fossify is GPL-3.0-only.** `Client/vendor/licenses/` holds the licence text and the source offer (exact upstream commit included); `CMakeLists.txt` copies both next to the EXE and into the installer. They must travel wherever the APK travels. The APK itself is downloaded at build time and checksum-verified **by hand** — not via `file(DOWNLOAD ... EXPECTED_HASH)`, which aborts the whole configure on a mismatch and leaves the bad file on disk for the next run to ship unverified. A missing APK stays a warning for developer builds, but `release.yml` configures with `-DPM_REQUIRE_FOSSIFY=ON`, which turns it into a `FATAL_ERROR`: the virtual display is on by default, so an installer without the launcher would send every user into the fallback with a green build to show for it. F-Droid moves superseded versions from `/repo/` into `/archive/`, so `PM_FOSSIFY_URLS` lists both — the `/repo/` URL is guaranteed to 404 once the next version appears.
...
- **Auto-pause while minimized** (`Settings::m_auto_pause_minimized`, default **on**): a window folded down to the taskbar shows nothing, but the phone keeps encoding and the WLAN keeps carrying ~20 Mbit/s. `Win32Window` fires `set_minimize_callback` on the `SIZE_MINIMIZED`/`SIZE_RESTORED` edge only (plain `WM_SIZE` also fires on every resize drag), and `main.cpp` reconciles from there. Rules:
  ...
  - **A teardown that folds the window down must not look like a human.** When the phone is switched off in virtual-display mode the screen-poll thread minimizes the window itself, and `ShowWindow(SW_MINIMIZE)` fires the very same `SIZE_MINIMIZED` edge. `pause_suspended` is raised *before* that task is posted; the minimize callback and the reconcile worker both bail out on it, and the poll thread clears `pause_wanted`/`auto_paused` after its `scrcpy.stop()`. Without it the worker could win the race, latch `auto_paused` on a stream it stopped itself and never lower it — and `auto_paused` is what the heartbeat loop counts as "still wanted", so the client would keep `POST /heartbeat` alive at a switched-off phone and hold its ADB open past the 60s watchdog. `start_connection` lowers the flag again.
...
- **The phone's auto-rotation is locked for the session** (`Settings::m_auto_rotate`, default **false** = locked). `start_stream` writes `accelerometer_rotation` to the saved preference on every connect, in both modes, because a picture on a PC monitor must not turn sideways when somebody moves the phone on the desk. This reverses the read-only rule of issue #46 — the menu toggle is now our own persisted setting, not a mirror of the phone's — but the phone's own value is read into `SavedAutoRotate` first and put back at app teardown and when the screen-poll sees the phone go dark, exactly like `SavedBrightness`. A click on the menu toggle drops the remembered value: teardown must not undo what the user just asked for on purpose. Both values live inside one **`SessionPhoneState`** behind a mutex, because four threads reach for them (the `start_stream` worker writes, the screen-poll restores and clears, the UI thread clears the rotation value, teardown restores the rest) and each carries a `std::string` — an unguarded write is a heap race, not a stale int. `restore_phone_state()` is the only way out: it copies under the lock, clears, and talks to ADB afterwards, so nothing is restored twice and the UI thread never waits on an `adb` child. Teardown restores **after** `background_tasks.join_all()`, otherwise it puts back a value the worker has not written yet and the phone stays locked.
- **Optional UHID keyboard** (`Settings::m_uhid_keyboard`, off by default): instead of `inject_text`, a virtual USB HID keyboard is attached to the device (scrcpy control messages 12 `UHID_CREATE` / 13 `UHID_INPUT` / 14 `UHID_DESTROY`, plus 15 `OPEN_HARD_KEYBOARD_SETTINGS`). Rules that keep it working:
  ...
  - **Wire format is version-specific.** The bundled server 4.1 expects `type(1) id(2 BE) vendor(2 BE) product(2 BE) name_len(1) name rd_size(2 BE) rd_data`; vendor/product are 0 for keyboards and mice (only gamepads carry real ids). Server 2.7 had no vendor/product fields at all, and the `name` field does not exist before 2.7. The layout is unchanged between 3.3.4 and 4.1. Verify against the vendored jar before changing it — see `VIRTUAL-DISPLAY.md` for the full 2.7 → 3.3.4 delta.
