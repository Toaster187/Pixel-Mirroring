...
- `Client/tests/control_messages_test.cpp` — same shape, pinning the scrcpy wire
  ...
  `START_APP`, `SET_DISPLAY_POWER`'s boolean payload, the session/config/key-frame flag
  bits, session-packet size parsing, and the ÷16 scroll scale. This is the one part of
...
- **Virtual display is an experiment, and it is on by default** (`Settings::m_virtual_display`, branch `virtual-display-dev-`). The PC gets its own Android display instead of the phone's picture. Three things are load-bearing and each was learned the hard way — the full record with measurements is in `VIRTUAL-DISPLAY.md`:
  - **The phone's own launcher for secondary displays is useless.** On a Pixel 9 / Android 17 its app drawer opens but tapping an app starts nothing, without even a refusal in the log. The mode therefore ships Fossify Launcher, starts it with `START_APP` and `vd_system_decorations=false`, and installs it during first-time setup over USB. Installing goes through `push_file` + `install_pushed_app`, deliberately **not** `install_app` with its `-g`. Three consequences: (1) with no launcher available `start_stream` falls back to plain mirroring and says so, because the alternative is handing the user a display that renders but starts nothing; (1a) everything that decides "is this a display of our own" must therefore ask the **session** (`ScrcpyClient::uses_new_display()`), never `Settings::m_virtual_display` — the setting only says what was wanted. `unlock_device_if_needed` runs beside `start_stream` and cannot know yet, so it takes the expectation as an argument and both connect paths call it a second time when the session fell back, or the user ends up looking at a mirrored lock screen nobody unlocks; (2) the launcher package is resolved per session into `ScrcpyClient::Config` and deliberately **never** written back to `settings.txt` — `main.cpp` holds a long-lived `Settings` copy and rewrites the whole file on every menu click, so a value stored there would live only until the next toggle. `ScrcpyClient::new_display_app()` is the authority.
  - **Fossify is GPL-3.0-only.** `Client/vendor/licenses/` holds the licence text and the source offer (exact upstream commit included); `CMakeLists.txt` copies both next to the EXE and into the installer. They must travel wherever the APK travels. The APK itself is downloaded at build time and checksum-verified **by hand** — not via `file(DOWNLOAD ... EXPECTED_HASH)`, which aborts the whole configure on a mismatch and leaves the bad file on disk for the next run to ship unverified.
...
- **The phone's auto-rotation is locked for the session** (`Settings::m_auto_rotate`, default **false** = locked). `start_stream` writes `accelerometer_rotation` to the saved preference on every connect, in both modes, because a picture on a PC monitor must not turn sideways when somebody moves the phone on the desk. This reverses the read-only rule of issue #46 — the menu toggle is now our own persisted setting, not a mirror of the phone's — but the phone's own value is read into `SavedAutoRotate` first and put back by `restore_auto_rotate` at app teardown and when the screen-poll sees the phone go dark, exactly like `SavedBrightness`. A click on the menu toggle drops the remembered value (`saved_auto_rotate = {}`): teardown must not undo what the user just asked for on purpose. **The teardown restore runs after `background_tasks.join_all()`, and that order is load-bearing** for both saved values: the worker that fills them in reads the phone over ADB first, so restoring before the join let a client closed inside that window skip the restore and then let the worker write the lock — a phone stuck with rotation off and nothing on screen to say why.
