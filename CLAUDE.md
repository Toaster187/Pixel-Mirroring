...
- **Optional UHID keyboard** (`Settings::m_uhid_keyboard`, off by default): instead of `inject_text`, a virtual USB HID keyboard is attached to the device (scrcpy control messages 12 `UHID_CREATE` / 13 `UHID_INPUT` / 14 `UHID_DESTROY`, plus 15 `OPEN_HARD_KEYBOARD_SETTINGS`). Rules that keep it working:
  ...
  - **Wire format is version-specific.** The bundled server 3.3.4 expects `type(1) id(2 BE) vendor(2 BE) product(2 BE) name_len(1) name rd_size(2 BE) rd_data`; vendor/product are 0 for keyboards and mice (only gamepads carry real ids). Server 2.7 had no vendor/product fields at all, and the `name` field does not exist before 2.7. Verify against the vendored jar before changing it — see `VIRTUAL-DISPLAY.md` for the full 2.7 → 3.3.4 delta.
