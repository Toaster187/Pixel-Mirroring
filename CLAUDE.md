...
- **Dropped files are transferred deliberately slowly**: files dragged onto the window go out through `AdbClient::push_file_paced` in 1 MiB pieces, and after each piece the transfer sleeps for exactly as long as that piece took. The stream therefore keeps at least half the airtime no matter how fast the WiFi is today. Do not "optimise" this back into a single `adb push` — the picture stutters for the whole transfer. Pieces grow under `<target>.pmpart` and only get the real name via `mv` at the end, so an aborted transfer cannot destroy an existing file. A dropped APK the user wants installed goes to `/data/local/tmp` and is unpacked there with `pm install`: `adb install` would send it a second time at full speed, and the package installer cannot read out of `/sdcard`.
- **Keyboard shortcuts**: navigation uses **Alt** (`Alt+B` back, `Alt+H` home, `Alt+S` app switch, `Alt+N` notification panel, `Alt+Q` quick settings, `Alt+Shift+N`/`Alt+Shift+Q` collapse) because Ctrl+C/V carry the clipboard and Ctrl+U/L unlock/lock. Alt keys arrive as `WM_SYSKEYDOWN`/`WM_SYSKEYUP`, not `WM_KEYDOWN`; only B/H/S/N/Q are claimed so `Alt+F4` still reaches `DefWindowProc`. B/H/S are Android keycodes and go through `set_key_callback`; N/Q are scrcpy control messages 5/6/7 and therefore ride the `MenuAction` path instead, like Ctrl+U/L do.
